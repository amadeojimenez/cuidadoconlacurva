...
let canvasID,
  ...
  // ctxDO2 = dotsCanvas.getContext("2d"),
  ...
  greent = getComputedStyle(document.documentElement).getPropertyValue(
    `--greenlee-t`
  ),
  red = getComputedStyle(document.documentElement).getPropertyValue(`--fred`),
  redt = getComputedStyle(document.documentElement).getPropertyValue(
    `--fred-t`
  ),
  blue = getComputedStyle(document.documentElement).getPropertyValue(`--blue`),
  bluet = getComputedStyle(document.documentElement).getPropertyValue(
    `--blue-t`
  ),
...
function init() {
  ...
  for (const player in players) {
    ...
    players[player].color = getComputedStyle(
      document.documentElement
    ).getPropertyValue(`--${player}`); //colors from css :root object
  ...
  // ctxDO2.lineWidth = borderWidth
  // ctxDO2.strokeStyle = yellow
  ctxDO.strokeRect(
    h / 2 + borderWidth / 2,
    h / 2 + borderWidth / 2,
    h / 4 - borderWidth,
...
function drawStart() {
  for (const player in players) {
    ...
    ctxDO.arc(
      players[player].x,
      players[player].y,
      (playerSize / 2 - 0.1) * players[player].powerup.size,
      0,
      r2d(360),
      true
    );
...
function draw() {
  ...
  if (achtung.sides != 0) {
    ctxDO.strokeStyle = `rgba(255, 55, 0, ${
      Math.abs((tFrame % 40) - 20) / 20
    })`;
  ...
  for (const player in players) {
    ...
    let prevprevPosX =
        players[player].x -
        mathCos(players[player].dir) *
          moveSpeed *
          players[player].powerup.speed,
      prevprevPosY =
        players[player].y -
        mathSin(players[player].dir) *
          moveSpeed *
          players[player].powerup.speed,
    ...
    if (players[player].powerup.reverse == 0) {
      if (players[player].powerup.side == 0) {
        ...
        ctxDO.fillStyle = `rgba(255, 255, 0, ${
          Math.abs((tFrame % 40) - 20) / 20
        })`; //flicker dot if side powerup
    } else {
      if (players[player].powerup.side == 0) {
        ctxDO.fillStyle = blue;
      } else
        ctxDO.fillStyle = `rgba(0, 0, 255, ${
          Math.abs((tFrame % 40) - 20) / 20
        })`; //flicker dot if side powerup
    }
    ...
    if (players[player].powerup.robot == 0) {
      ...
      if (players[player].turnL) {
        if (players[player].powerup.reverse == 0) {
          players[player].dir -=
            turnSpeed / Math.pow(players[player].powerup.size, 0.3);
        } else
          players[player].dir +=
            turnSpeed / Math.pow(players[player].powerup.size, 0.3);
      }
    ...
    if (
      players[player].bridgeFrame <
      tFrame -
        (bridgeSize / players[player].powerup.speed) *
          players[player].powerup.size
    ) {
    ...
    let pxFront = Math.round(
      ...
      pxLeft = Math.round(
        players[player].x +
          mathCos(players[player].dir - r2d(55)) *
            hitboxSize *
            players[player].powerup.size
      ),
      pyLeft = Math.round(
        players[player].y +
          mathSin(players[player].dir - r2d(55)) *
            hitboxSize *
            players[player].powerup.size
      ),
      pxRight = Math.round(
    ...
    for (let i = 0; i < achtung.powerupsOnScreen.length; i++) {
      if (
        (ctxPHImgData[pixAlphaFront - 1] == i * 3 + 1 &&
          ctxPHImgData[pixAlphaFront - 2] == i * 3 + 2 &&
          ctxPHImgData[pixAlphaFront - 3] == i * 3 + 3) ||
        (ctxPHImgData[pixAlphaLeft - 1] == i * 3 + 1 &&
          ctxPHImgData[pixAlphaLeft - 2] == i * 3 + 2 &&
          ctxPHImgData[pixAlphaLeft - 3] == i * 3 + 3) ||
        (ctxPHImgData[pixAlphaRight - 1] == i * 3 + 1 &&
          ctxPHImgData[pixAlphaRight - 2] == i * 3 + 2 &&
          ctxPHImgData[pixAlphaRight - 3] == i * 3 + 3)
      ) {
    ...
    if (!players[player].bridge) {
      ...
      if (players[player].powerup.invisible == 0) {
        ...
        if (players[player].powerup.robot == 0) {
          ...
          if (
            ctxTHImgData[pixAlphaFront] == 255 ||
            ctxTHImgData[pixAlphaFront2] == 255 ||
            ctxTHImgData[pixAlphaLeft] == 255 ||
            ctxTHImgData[pixAlphaRight] == 255
          ) {
            givePoints(players[player]);
            continue;
          }
        } else {
          if (ctxTHImgData[pixAlphaFront] == 255) {
...
function checkGameState() {
  ...
  if (achtung.gameEnded) {
    if (
      achtung.scoreArray[achtung.scoreArray.length - 1][1] >= achtung.pointGoal
    ) {
      if (
        achtung.scoreArray[achtung.scoreArray.length - 1][1] -
          achtung.scoreArray[achtung.scoreArray.length - 2][1] >
        1
      ) {
...
function doPowerups(puPlayer, index) {
  ...
  if (powName == "o_random") {
    ...
    const activeSnakes = snakes.filter((snake) => !!players[snake].active);
    snakes.forEach((snake, index) => {
      // snake?
      // random number between 0 and 5
      console.log("snake", snake);
      const random = Math.floor(Math.random() * activeSnakes.length);
      const newSnake = snakes[random];
      console.log("newSnake", newSnake);
      players[snake].color = getComputedStyle(
        document.documentElement
      ).getPropertyValue(`--${newSnake}`);
    });
  }
  if (powName == "g_slow") {
  ...
  if (powName == "g_thin") {
    ...
    players[puPlayer].powerup.toClear[index] = setTimeout(
      () => (players[puPlayer].powerup.size *= 2),
      gTimeout
    );
  }
  if (powName == "g_robot") {
  ...
  if (powName == "r_thick") {
    for (const otherPlayers in players) {
      if (otherPlayers != puPlayer) {
        ...
        players[otherPlayers].powerup.toClear[index] = setTimeout(
          () => (players[otherPlayers].powerup.size *= 0.5),
          rTimeout
        );
      }
    }
...
const powerupDraw = () => {
  ...
  for (let i = 0; i < achtung.powerupsOnScreen.length; i++) {
    ...
    if (pow.charAt(0) == "g" || pow.charAt(0) == "r" || pow.charAt(0) == "b") {
      ...
      ctxPV.moveTo(
        Math.cos(r2d(line1[1])) * iconSize,
        Math.sin(r2d(line1[1])) * iconSize
      );
      ctxPV.lineTo(
        Math.cos(r2d(line1[0])) * iconSize,
        Math.sin(r2d(line1[0])) * iconSize
      );
      ctxPV.fill();
      ...
      ctxPV.lineTo(
        Math.cos(r2d(line2[0])) * iconSize,
        Math.sin(r2d(line2[0])) * iconSize
      );
      ctxPV.fill();
      ...
      ctxPV.lineTo(
        Math.cos(r2d(line2[1])) * iconSize,
        Math.sin(r2d(line2[1])) * iconSize
      );
      ctxPV.fill();
    }
...
const mathCos = (n) => {
  return round100(Math.cos(n));
};
const mathSin = (n) => {
  return round100(Math.sin(n));
};

//returns pixel index for alpha value in raw pixel data array
const getAlphaIndexForCoord = (x, y, width) => {
  return y * (width * 4) + x * 4 + 3;
};
...
function calcRandomStartPos() {
  for (const player in players) {
    players[player].x = map(
      calcRandomInt(h),
      0,
      h,
      borderWidth * 10,
      h - borderWidth * 10
    ); //map to avoid instant death
    players[player].y = map(
      calcRandomInt(h),
      0,
      h,
      borderWidth * 10,
