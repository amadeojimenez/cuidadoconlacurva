...
function setLeftButton(name, w, key, keyName) {
  ...
  document
    .querySelector(`.player_wrapper.${name} .key_wrapper_left .key_button`)
    .classList.add("hidden");
  document.querySelector(
    `.player_wrapper.${name} .key_wrapper_left .key_text`
  ).textContent = keyName.replace("Arrow", "").toUpperCase();
...
function setRightButton(name, w, key, keyName) {
  ...
  document
    .querySelector(`.player_wrapper.${name} .key_wrapper_right .key_button`)
    .classList.add("hidden");
  document.querySelector(
    `.player_wrapper.${name} .key_wrapper_right .key_text`
  ).textContent = keyName.replace("Arrow", "").toUpperCase();
...
document
  .querySelector("#gamemode_wrapper p:nth-child(1)")
...
document
  .querySelector("#gamemode_wrapper p:nth-child(2)")
  .addEventListener("click", (u) => {
    u.path[0].classList.add("underline");
